...
- **15** Symmetric-LRMC cost-recovery mechanism (supports §6 prose)
- **16** Host-country WACC channel mechanism (supports §7.1 prose)
- **17** Bilateral sovereignty premium λ_ij, training side (supports §6.2 prose)
- **18** Construction-cost OLS (Table A7)
- **19** Recomputes and writes all eleven tables to `output/table*.csv`

The constant tables in steps 15–17 and 19 (the 56 cost-recovery prices,
carbon prices/intensities, OECD scope, cross-subsidies, income-group WACC
bands, geopolitical-bloc matrix, EU/APEC-CBPR/DEPA membership, sanctioned and
developing-country sets) are transcribed from the paper's Python generator.

### Fidelity vs. the published tables

Every analytical exhibit is reproduced **cell-for-cell**. Step 19 recomputes
Tables 3, A1, A2 and A3 with the exact constants, equilibrium algorithm,
sorting, units and number formats of the published generator and writes the
same cell strings the paper prints (including the EE/IE/DD/II regime letters
and the sanction `*` / developing `†` flags). Step 14's scenario runner
re-runs the **full** DCF cash-flow model for every row of Table A6 and asserts
that its base case equals the main DCF.

| Published table | Stata reproduction | Status |
|---|---|---|
| Table 3 (4 spec blocks, top 25) | step 19 | ✅ exact, all cells |
| Table A1 (calibration params, 85 countries) | step 19 | ✅ exact, all cells |
| Table A2 (rankings, all 85 countries) | step 19 | ✅ exact, all cells |
...
channel, bilateral training sourcing) that document each specification and
support in-text §6–§7 numbers. The exported tables do **not** depend on them:
step 19 recomputes everything it exports directly from `data/` so that each
exhibit reproduces the paper exactly. Step 17's own summary statistics remain
a training-side proxy (see its header) and are not used in any exhibit.
...
| 12 | `12_sensitivity.do` | **`output/sensitivity_results.dta`** | parameter-robustness scenarios (§7.1 prose; Table A3 itself is recomputed in step 19) |
...
| 15 | `15_symmetric_lrmc.do` | `temp/symmetric_lrmc.dta` | symmetric-LRMC mechanism (§6 prose; p_T≈$1.60) |
| 16 | `16_wacc_channel.do` | `temp/wacc_channel.dta` | WACC-channel mechanism (§7.1 prose) |
| 17 | `17_bilateral_lambda.do` | `temp/bilateral_lambda.dta` | bilateral λ training side (§6.2 prose) |
| 18 | `18_construction_regression.do` | **`output/tableA7_construction_regression.csv`** | Table A7 |
| 19 | `19_export_tables.do` | **`output/table{1,2,3,A1–A8}_*.csv`** | all paper tables, recomputed paper-exactly in this step (see §2 fidelity table) |
