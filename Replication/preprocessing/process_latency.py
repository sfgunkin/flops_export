...
process_latency.py
==================
Generates the analysis-ready input  data/country_pair_latency.csv
from raw WonderNetwork bilateral ping measurements.

What it does
------------
1. Reads the WonderNetwork server directory (id -> city/country/coords).
2. Streams the raw ping table (~3M source->destination round-trip-time rows),
   maps each endpoint to a country (ISO3), filters out implausible values
   (<=0 or >2000 ms), and accumulates latencies by country pair.
3. Aggregates each ordered country pair to mean/median/min/p95/max RTT and a
   ping count.

Inputs:
  - data/wondernetwork_servers.csv                      (included in package)
        server id, name, city (title), country, latitude, longitude.
  - data/raw/latency/wondernetwork_pings.csv.gz         (~49 MB, NOT redistributed)
        raw bilateral ping measurements with columns source, destination, avg.
        See README §"Data inputs" and the Data Availability Statement.

Output:
  - data/country_pair_latency.csv
        columns: iso3_from, iso3_to, country_from, country_to,
                 avg_ms, median_ms, min_ms, p95_ms, max_ms, n_pings

Run from anywhere:
    python preprocessing/process_latency.py
Paths are resolved relative to this file.

Dependencies: numpy  (see preprocessing/README.md).
"""
...
DATA = ROOT / "data"
SERVERS_FILE = DATA / "wondernetwork_servers.csv"
PINGS_FILE = DATA / "raw" / "latency" / "wondernetwork_pings.csv.gz"
...
if not SERVERS_FILE.exists():
    raise SystemExit(f"Server directory not found at {SERVERS_FILE}")
servers = {}
...
if not PINGS_FILE.exists():
    raise SystemExit(
        f"Raw ping file not found at {PINGS_FILE}\n"
        "This ~49 MB file is not redistributed in the package (see the Data "
        "Availability Statement). Place wondernetwork_pings.csv.gz there to "
        "regenerate country_pair_latency.csv; the analysis-ready CSV is already "
        "shipped in data/ so the Stata pipeline runs without it."
    )
...
with gzip.open(PINGS_FILE, "rt") as f:
