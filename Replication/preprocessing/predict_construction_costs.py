"""
predict_construction_costs.py
=============================
Generates the analysis-ready input  data/predicted_construction_costs.csv:
per-country data-center construction cost (US$/watt of IT load) for every
country, predicted from a log-linear regression trained on the observed
Turner & Townsend Data Centre Construction Cost Index (DCCI) 2025 markets.

Regression:
  ln($/W) = a + b1*ln(GDP_pcap) + b2*ln(pop) + b3*urban_share
            + b4*seismic_high + region dummies
Trained on DCCI 2025 (52 markets -> 37 unique countries); a Duan smearing
factor corrects the log retransformation when predicting levels.

Inputs (all shipped in the package):
  - data/dcci_2025_construction_costs.csv   (Turner & Townsend DCCI 2025, $/W)
  - data/wb_gdp_per_capita_ppp_2023.csv     (World Bank WDI)
  - data/wb_population_2023.csv             (World Bank WDI)
  - data/wb_country_regions.csv             (World Bank region classification)
  - data/wb_urban_share_2023.csv           (World Bank WDI)
  - data/seismic_zones.csv                  (high-seismic-hazard country flag)

Output:
  - data/predicted_construction_costs.csv

Run from anywhere:  python preprocessing/predict_construction_costs.py
