"""
Produce flop_trade_model_v34.docx from v8.docx.

v34: Targeted WBER-revision edits (question/answer framing, policy levers,
     welfare-figure locks) per v33_targeted_revision_edits_1.md.
v33: Symmetric LRMC cost-recovery (addresses referee Issue 4.3).
  - SUBSIDY_ADJ extended from 13 developing countries to symmetric p_E vector
    covering all 85 countries: 13 keep IMF-based LRMC (v32), 43 OECD/high-income
    receive carbon-price adder + documented cross-subsidy add-back, 29
    middle-income developing economies retain observed tariffs.
  - Carbon adders use EMBER 2024 grid carbon intensity × 2024 ETS auction prices
    (EU ETS $70.94, UK $47.32, Canada $58.48, US weighted $3.81, NZ $39.50,
    SGP $18.60; Korea/Japan/Australia/Israel/Chile/Mexico zero).
  - Cross-subsidy add-backs: DEU $0.038 (EEG), FRA $0.015 (ARENH), USA $0.015
    (Borenstein 2012, Davis-Hausman 2016), KOR $0.020 (KEPCO), ESP/ITA/NLD/BEL
    $0.010 each, JPN zero (post-2016 liberalization).
  - New top-5: Kyrgyzstan, Ethiopia, Kosovo, Canada, China (was KGZ, CAN, ETH,
    XKX, TJK). Canada drops rank 2→4 on $0.008/kWh carbon adder. Poland +21
    positions, Germany +10, USA +10, France +12; Nordic/Swiss stable (±2).
  - New Appendix E.1 "Symmetric LRMC Construction" documents methodology.
  - Table A1 gains a new Cost-Rec. p_E (symmetric) column (keeps the existing
    Cost-Rec. p_E column for reader comparison). Table 3 spec (2) replaced.

v32: New Figures 1 (model structure) and 1b (regime feasibility grid).
  - Figure 1 replaced: calibration_strategy → model_structure diagram
  - Figure 1b added: 3×3 regime feasibility grid (Prop 1)
  - In-text Figure 1 reference updated to match new content

v31: Integrate user edits — new FDI evidence paragraphs, new references:
  - ¶13: FDI examples (Google, Microsoft, CloudHQ, OpenAI) + Aykut/Straub citations
  - New ¶21: "Concurrent evidence on FDI geography" after lit review
  - New ¶80: "Recent FDI data confirm" after cost-investment gap
  - New ¶86: "China illustrates DD regime" after demand centers
...
v30: Drop ξ efficiency index entirely — rely solely on bilateral λ_{ij}:
  ...
  - Form B (c_eff = ρ + (c_cr − ρ)/ξ) removed from equilibrium and inference
  - All equilibrium passes use cost-recovery supply stack directly
  - Table 3a loses spec (3) efficiency-adjusted; Table 3b renumbered (4)-(7)→(3)-(6)
  - Figure 2 (efficiency rank scatter) removed
  - Sensitivity simplified to 3 scenarios (baseline, low ρ, high ρ)
  - Delivered cost simplified: P_s(j,k) = (1+λ_{jk})·(1+τ_s·l_{jk})·c_j

v29: Fresh pipeline numbers — explicit η, no residual, no ξ floor:
  - rho_net = ETA (explicit $0.15 networking, replaces residual back-calculation)
  - Dynamic top-25 selection (sorted by rank_cr, no hardcoded list)
  - Inline sensitivity computation (3 scenarios, no external xlsx)
  - XI_FLOOR / compute_xi_eff removed (dead code)
  - Section 6.1 prose updated for new rankings (zero developing in top 15)
...
# Anonymized-submission mode (double-blind journals, e.g. JITED). When set via
# `python add_calibration_v34.py --anon`, the title page omits the author name,
# the acknowledgement/disclaimer footnote, and the version stamp; the output is
...
# Symmetric LRMC cost-recovery ($/kWh) — v33 (addresses referee Issue 4.3)
# Covers ALL 56 countries needing adjustment (13 developing + 43 OECD/high-income);
# the other 29 middle-income developing economies retain observed tariffs.
#
# Developing-country entries (13): IMF (2025) explicit fossil-fuel subsidies +
#   Lazard (2025) LCOE v17 — unchanged from v32.
#
# OECD / high-income entries (43): observed tariff + 2024 carbon-price adder
#   (EMBER 2024 grid CI × 2024 ETS auction price) + documented cross-subsidy
#   add-back. Carbon regimes: EU ETS $70.94, UK ETS $47.32, Canada backstop
#   $58.48, US CA/WA+RGGI weighted $3.81, NZ ETS $39.50, Singapore $18.60;
#   Korea/Japan/Australia/Israel/Chile/Mexico zero (nominal <$10 effective).
#   Cross-subsidies: DEU $0.038 (EEG), FRA $0.015 (ARENH), USA $0.015
#   (Borenstein 2012, Davis-Hausman 2016), KOR $0.020 (KEPCO below cost),
#   ESP/ITA/NLD/BEL $0.010 (Eurostat IB6).
# Three overlapping HI non-OECD (QAT, SAU, ARE) retain IMF-based treatment.
#
# Full provenance in work/lrmc_symmetric/p_E_symmetric.csv and Appendix E.1.
SUBSIDY_ADJ = {
    # --- IMF-based LRMC (13 developing countries; unchanged from v32) ---
    ...
    # --- Symmetric LRMC (43 OECD + EU non-OECD + Singapore) ---
...
def run_sensitivity(cal, omega, dc_k, k_bar, sanctioned):
    """v30: Inline sensitivity computation (3 scenarios, no ξ).

    Recomputes cost-recovery rankings under alternative hardware cost (ρ).
    Returns list of scenario result dicts for Table A3.
    """
    # 3 scenarios for Table A3
    scenario_defs = [
        ('Baseline (\u03C1=$1.36)',
         '\u03C1=$1.36', RHO),
        ('Low hardware share (\u03C1=$1.30)',
         '\u03C1=$1.30 (\u22124.4%)', 1.30),
        ('High hardware share (\u03C1=$1.42)',
         '\u03C1=$1.42 (+4.4%)', 1.42),
    ]
    ...
    for label, param_change, rho_val in scenario_defs:
        ranked = []
        for r_row in cal:
            iso = r_row["iso3"]
        ...
        if not baseline_ranks:
            baseline_ranks = {d["iso"]: d["rank_cr"] for d in ranked}

        # Dev in top 15
        dev_top15 = sum(1 for d in ranked[:15] if d["iso"] in DEVELOPING)
        # Max cost spread: max (c_cr - c_cr_min) / c_cr_min
        c_min = ranked[0]["c_cr"]
        max_spread = (ranked[-1]["c_cr"] - c_min) / c_min * 100 if c_min > 0 else 0
...
def write_title_and_abstract(doc, body, all_el, hmap, demand_data=None):
    ...
    r_author = author_p.add_run('' if ANON else 'Michael Lokshin')
...
def write_introduction(doc, body, hmap):
    ...
    p.add_run(
        ...
        'resources. The compute used to train machine-learning '
    ...
    # Para 6b (v34, Edit A1): the Kyrgyzstan paradox — the question the
    ...
    # Para 11: Calibration findings preview (v34, Edit A2: verdict + the two
    ...
    p.add_run(
        ...
        'frictions. Under cost-recovery pricing, several developing countries '
        'rank among the lowest-cost producers. However, when bilateral '
        'sovereignty frictions are introduced, trust deficits with OECD buyers '
        'eliminate nearly all of their export potential. Financing costs '
        'reinforce this result: a 10-percentage-point WACC gap between OECD '
        'hyperscalers and locally financed developing-country operators adds '
        'about $0.29 per GPU-hour to unit cost, roughly four times the '
...
def write_literature(doc, body, hmap):
    ...
    p, cur = mkp(doc, body, cur)
    p.add_run(
        'The closest precedent to our model is IT services offshoring, which is '
        'labor-intensive and skill-biased (Blinder 2006). FLOP exporting differs '
        'from traditional IT services offshoring in factor intensity. It is '
        'capital- and energy-intensive rather than labor-intensive, so likely '
        'exporters are energy-rich countries rather than labor-abundant ones. '
        'The structural difference is that GPUs, the main traded input, are '
        'priced globally. Cross-country cost variation therefore comes primarily '
        'from electricity and facility costs, which together account for only a '
        'small share of unit cost. Comparative advantage in compute is '
        'consequently narrower and more fragile than in services offshoring, '
        'where labor-cost differences are larger.'
    )
    ...
    p.add_run(
        ...
        '(high-income countries hold 77 percent of colocation capacity), '
        'but does not offer a formal framework linking costs to trade patterns. '
        'Biglaiser et al. (2024) survey the cloud market in IO, and '
...
def write_production_technology(doc, body, hmap):
    ...
    p.add_run(
        ' (PUE), the ratio of total facility energy consumption to IT equipment '
        'energy consumption. The cost per GPU-hour in country '
    )
    omath(p, [_v('j')])
    p.add_run(' is:')
    make_footnote(
    ...
    make_footnote(
        p,
        'China is developing an alternative domestic chip stack based on '
        'Huawei\u2019s Ascend series (910B/910C) and other domestic accelerators. '
        'If these achieve comparable FLOPs per watt at lower prices, China\u2019s '
        'effective \u03C1 could diverge from the NVIDIA-based benchmark used '
        'here, potentially improving its cost position despite export controls.',
        10,
    )
    p.add_run(
    ...
    p, cur = mkp(doc, body, cur)
    p.add_run(
        'The cost structure is Ricardian: countries differ in production costs '
        'because they differ in technology and input prices. '
        'The intuition for resource-rich comparative advantage, however, is '
        'closer to Heckscher\u2013Ohlin with countries exporting goods intensive '
        'in their abundant factors (Heckscher 1919, Ohlin 1933). '
...
def write_trade_costs(doc, body, hmap):
    ...
    p.add_run(
        ' (denoted T) include batch workloads such as model training, fine-tuning, '
    ...
    omath(p, [_msub('l', 'jk')])
    p.add_run(
        ', is the round-trip time for a data packet to travel from the seller country '
    )
    omath(p, [_v('j')])
    p.add_run(' to buyer country ')
    omath(p, [_v('k')])
    p.add_run(
    ...
    p.add_run(
        '). The bilateral premium is modeled as a function of '
    ...
    omath(p, [_msub('c', 'j')])
    p.add_run(
        ' is the unit cost of producing one GPU-hour in country '
    )
    omath(p, [_v('j')])
    p.add_run(
...
def renumber_sections(hmap):
    ...
    # v20 structure: 1=Intro, 2=Lit, 3=Model(3.1,3.2), 4=Equil Props,
    # 5=Data, 6=Calib, 7=Robustness, 8=Concl
    # v8 headings: 1→3 (Model), 1.1→3.1, 1.2→3.2, 2→4 (Equil Props),
    # 4→6 (Calib), 5→8 (Concl)
    # Section 3 (Make-or-Buy) content is absorbed; heading removed by write fns
    renumber = [
    ...
    if '4' in hmap:
        for t in hmap['4'].findall(f'.//{qn("w:t")}'):
            if t.text and 'Discussion' in t.text:
                ...
                break
    # Rename Section 3 heading: "Model Setup" → "Model of Compute Production and Trade"
    if '1' in hmap:
        for t in hmap['1'].findall(f'.//{qn("w:t")}'):
            if t.text and 'Model Setup' in t.text:
                t.text = t.text.replace('Model Setup',
                                        'Model of Compute Production and Trade')
                break
...
def write_demand(doc, body, hmap, demand_data):
    ...
    p.add_run(
        ...
        'latency-insensitive and freely offshorable. The effective offshorable '
        'share may be smaller as intermediate workloads (agentic inference, '
        'fine-tuning) grow.'
    )
    make_footnote(
        p,
        'Emerging workload categories, notably agentic inference (long-running, '
        'multi-step reasoning tasks) and fine-tuning (rapid iterative retraining '
        'on proprietary data), occupy a middle ground, tolerating moderate '
        'latency but requiring sustained GPU allocation and proximity to data. '
        'Using installed capacity to proxy demand is a static assumption; '
        'endogenizing demand, for instance, proportional to GDP or digital '
...
def write_sourcing_and_equilibrium(doc, body, hmap, demand_data):
    ...
    omath(p, [_v('k')])
    p.add_run(
        ' imports training whenever the world price, after adding the bilateral '
        'sovereignty premium, is lower than the price of producing domestically: '
    )
    ...
    omath(p, [_msub('p', 'T')])
    p.add_run(
        ' is the competitive world training price. In the capacity-constrained '
        'equilibrium, the cheapest producer supplies up to its capacity, then '
        'the next cheapest producer enters, and so on until demand is met. The marginal '
        'training exporter '
    )
    omath(p, [_msub('m', 'T')])
    p.add_run(
    ...
    p.add_run(
        ...
        'That activity is the least profitable use that still receives capacity. '
    ...
    p.add_run(
        ...
        'produce and export compute. Whether these countries attract '
        'investment also depends on agglomeration economies, hyperscaler market '
        'structure, and network connectivity (Krugman 1991). In practice, AWS, Azure, '
        'and Google Cloud dominate the cloud compute market, and their location '
        'choices reflect scale economies and colocation effects that the '
        'competitive model does not capture. The capacity ceilings '
    )
    omath(p, [_mbar_sub('K', 'j')])
    p.add_run(
        ' partially capture the resulting gap between cost-based potential and realized '
        'investment; Section 7 discusses these limitations in more detail.'
    )
...
def write_equilibrium_properties(doc, body, hmap, demand_data):
    ...
    # Remove everything from after Section 4 heading to before old Calibration
    # heading; drops old Section 4 content and old Section 5 (Make-or-Buy).
    for el in all_now[s4i + 1:s4_next_i]:
        body.remove(el)
    cur = s4

    # Also rename heading from "Comparative Advantage" to "Equilibrium Properties"
    for t in s4.findall(f'.//{qn("w:t")}'):
        if t.text and 'Comp' in t.text:
            t.text = (
                t.text
                .replace('Comparative Advantage', 'Equilibrium Properties')
                .replace(' Results', '')
            )
            break
    ...
    p, cur = mkp(doc, body, cur)
    p.add_run(
        'This section derives the properties of the capacity-constrained '
        'equilibrium introduced in Section 3. Full derivations are shown in Appendix B.'
    )
    ...
    p.add_run(
        ...
        'import inference. The sovereignty premium that justifies domestic '
        'training also justifies domestic inference production, which faces '
        'latency degradation when imported. A country that produces training '
        'domestically while exporting inference would need both a strong '
        'domestic-sovereignty motive and very low costs. The model\u2019s cost '
        'ordering rules out that combination. '
    )
    ...
    r.italic = True
    p.add_run(
        'In the training market, define the Herfindahl\u2013Hirschman Index (HHI) '
        'as a measure of market concentration that is equal to the sum of '
    ...
    p.add_run(
        ...
        'bounded by the capacity ceiling '
    )
    omath(p, [_msub('K', 'T,j'), _t(' \u2264 '), _mbar_sub('K', 'j')])
    p.add_run(
        '. Without capacity constraints, the cheapest producer captures all '
        'training demand and '
    )
    ...
    p.add_run(
        ...
        'exporters. When low-cost producers reach capacity '
        'limits, residual demand shifts to higher-cost suppliers, reducing '
    ...
    r.italic = True
    p.add_run(
        'A country produces AI training domestically only when the sovereignty '
        'premium is large enough to offset the higher domestic cost. '
        'Formally, country '
    )
    omath(p, [_v('k')])
    ...
    make_footnote(
        p,
        'For large-demand countries (e.g., the United States with 43% of global '
        'compute demand), the observed domestic production may partly reflect '
        'scale economies rather than sovereignty preferences. The switching '
        'threshold \u03bb\u2096* conflates the home market effect with the '
        'sovereignty premium; disentangling the two would require a model with '
        'increasing returns, which lies outside the present framework.',
        22,
    )
    ...
    fn9_t2.text = (
        ' to every country with positive compute demand would export training '
        '(which is latency-insensitive) yet be unable to serve any inference '
        'market, a \u201cpure training exporter\u201d regime not listed in '
        'Table 1. This case is empirically negligible in the calibration: any '
        'country remote enough to have an empty latency cone (e.g., a '
        'mid-Pacific island) typically also faces prohibitive governance and '
        'construction cost penalties that exclude it from the training market.'
    )
    p.add_run(
        ' Training exporters therefore also export inference to nearby demand '
        'centers, provided that latency remains below the threshold '
    )
    omath(p, [_mbar('l')])
    p.add_run(
...
def write_data_section(doc, body, hmap, demand_data):
    ...
    p.add_run(
        ...
        'country-specific costs and bilateral frictions. Calibrating the '
        'production-cost and trade-cost parameters in equations '
        '(1)\u2013(4) requires data on electricity prices, temperatures, '
        'construction costs, bilateral latencies, and bilateral sovereignty '
        'frictions (geopolitical alignment, regulatory compatibility, and '
        'sanctions). '
    )
    ...
    p.add_run(
        ...
        'For non-European countries, prices are sourced from national '
        'regulatory tariff sheets and secondary sources. These include the U.S. '
        'Energy Information Administration (EIA 2025) for the United States, '
        'KEPCO for South Korea,'
    )
    make_footnote(
        p,
    ...
    p.add_run(
        ' national utility tariffs for Central Asian countries, and '
        'GlobalPetrolPrices (2025) for the remaining countries. All prices are '
        'converted to $/kWh at 2024 exchange rates.'
    )
    ...
    p.add_run(
        ...
        '(Hersbach et al. 2020) as the average monthly maximum in the three '
        'warmest months, aggregated across populated grid cells. '
    )
    p.add_run(
        'Construction costs per watt of IT capacity are from the Turner & Townsend '
        'Data Center Construction Cost Index 2025 (Turner & Townsend 2025), for 37 '
        'countries. For the remaining countries, costs are predicted '
        '(Appendix\u2009E). Since construction is only 3\u20136 percent of total per-GPU-hour '
    ...
    add_italic(p, 'Latency. ')
    p.add_run(
        'Inter-country round-trip latency is measured using WonderNetwork\u2019s '
        'global ping dataset (WonderNetwork 2024). For each country pair, the '
        'median round-trip time (RTT) in milliseconds is used. Domestic latency '
        'defaults to 5 ms where no intra-country measurement is available.'
    )
    ...
    p.add_run(
        ' For smaller markets, capacity is estimated from facility counts '
        '(Cloudscene 2025) and regional averages.'
    )

    # Calibration approach note
    p, cur = mkp(doc, body, cur)
    p.add_run(
        'Bilateral compute-trade data are not yet available, so we calibrate '
        'the model from hardware prices, electricity tariffs, and facility-cost '
        'coefficients rather than estimating it from observed trade flows. The '
        'calibration identifies the conditions under which FLOP exporting becomes '
        'viable. It also provides a framework that can be taken to gravity-style '
        'estimation as transaction-level data emerge.'
    )
...
def write_calibration(doc, body, hmap, cal, reg, n_eca, n_total, demand_data):
    ...
    p.add_run(
        ' per \u00b0C is estimated from cross-sectional variation in PUE '
        'across data center locations with different cooling loads '
        '(Liu et al. 2023). The threshold '
    )
    omath(p, [_mbar('\u03B8'), _t(' = 15\u00b0C')])
    p.add_run(
        ' is the outdoor temperature above which mechanical cooling '
        'is needed, below which free-air cooling suffices. These assumptions '
        'yield PUE values from 1.08 (Iceland, Scandinavia) to 1.41 (UAE), '
        'consistent with the industry average of 1.56 (Uptime Institute 2024).'
    )
    make_footnote(
        p,
        'Capping PUE at 1.20 (simulating universal liquid cooling) yields a '
        'Kendall rank correlation of 0.96 with the baseline rankings. The top '
        'five countries are unchanged, and the maximum rank shift is six '
        'positions. Gulf states and North Africa gain the most (UAE moves from '
        '26th to 20th, Qatar from 15th to 11th), but the effect is small '
        'because electricity prices, not cooling, dominate cross-country cost '
        'variation.', 13,
    )
    ...
    p.add_run(
        '/hr. This amortization is straight-line and includes no cost of '
        'capital. Columns (1)\u2013(3) of Table 3 therefore hold financing '
        'costs at zero, so the cross-country spread reflects only electricity, '
        'climate, and construction; column (4) adds country-specific financing '
        'through the WACC adjustment of Section 7.2. '
    ...
    p.add_run(
        ...
        'Europe and East Asia) inflates inference cost by 8 percent, '
    ...
    p.add_run(
        '. The implied premium for non-adversarial services trade remains '
        'below Benz and Jaax (2020) 16 percent upper bound for regulatory barriers '
    ...
    p.add_run(' Table 3 also reports results under a uniform premium ')
    omath(p, [_v('\u03BB'), _t(f' = {LAMBDA * 100:.0f} percent')])
    ...
    p, cur = mkp(doc, body, cur)
    p.add_run(
        'Under observed electricity tariffs, the cheapest producer in our '
        f'sample of 85 countries is {cheapest["country"]}, '
    ...
    p.add_run(
        '). This ranking, however, overstates the role of production-cost advantage. '
    ...
    make_footnote(
        p,
        'For gas exporters (Iran, Turkmenistan, Algeria, Qatar), the '
        'calibration uses combined-cycle gas generation at export-parity fuel '
    ...
    p.add_run(' extends the ranking to all countries).')

    p, cur = mkp(doc, body, cur)
    # v30: dynamically find small-capacity countries in top 20 and large-capacity outliers
    _dc_k = demand_data["dc_k"]
    ...
    p.add_run(
        ...
        'Thus, the cost ranking should be interpreted as the set '
        'of feasible exporters, not as a prediction of realized investment.'
    )

    # v31: FDI data confirming cost-vs-investment mismatch + bilateral channel
    p, cur = mkp(doc, body, cur)
    p.add_run(
        'Data-center FDI patterns support this interpretation. Among EMDEs, the '
        'largest recipients of data-center investment are Malaysia, India, and '
        ...
        '2026). Most investment flows occur between '
        'geopolitical allies. This pattern is consistent with the bilateral '
        'sovereignty premium and suggests that cost alone does not determine '
        'where capacity is built (Straub et al. 2026). Econometric evidence '
        'shows that electricity costs do not predict data-center location once '
        'market size and the regulatory environment are controlled for '
        '(Caoui and Steck 2025).'
    )
    ...
    p.add_run(
        ...
        'Economies with advanced AI and cloud strategies\u2014such '
        ...
        'Indonesia and Viet Nam (Katz et al. 2025). In the '
        'model, institutional quality stands in for the complementary '
        ...
        'digital strategies with energy-sector '
        ...
        'These cases are consistent with the model\u2019s prediction that moderate '
        'costs can attract investment when paired with credible institutions '
        '(Aykut et al. 2026).'
    )
    ...
    omath(p, [_msub('\u03BB', 'ij')])
    p.add_run(
        ' reshapes trade patterns along geopolitical lines. '
        'Between allies with mutual data adequacy '
        'agreements (e.g., EU member states), the effective premium is near zero. '
        'For geopolitical adversaries, it is effectively infinite\u2014the United States '
    ...
    p.add_run(
        ...
        'The bilateral sovereignty premium has the strongest effect on inference. '
        'Within Europe, latency raises costs by 1\u20133 percent, so a '
        'small sovereignty premium can shift sourcing from foreign suppliers to '
        'domestic production.'
    )
    ...
    for iso, share in top5_inf:
        ...
        pct = share * 100
        fmt = f'{pct:.0f} percent' if pct >= 1 else f'{pct:.1f} percent'
    ...
    p.add_run(
        ...
        'to domestic training. The remaining export market is '
    ...
    p.add_run(
        ...
        f'{sum(s * 100 for _, s in top5_inf):.0f} percent of cross-border inference demand '
    ...
    if kgz_total > 0:
        ...
        p.add_run(
            f'Among developing countries, Kyrgyzstan captures {kgz_total:.0f} percent of global '
        ...
        if _iso in _dev and _iso != 'KGZ' and _share > 0.01:
            ...
            if _n_served > 0:
                ...
                p.add_run(
                    ...
                    f'accounting for {_share * 100:.0f} percent of global inference demand. '
                )
            break
    ...
    p.add_run(
        'The model\u2019s predictions differ across major AI demand centers because '
        'each faces a distinct latency geography. '
        'For the United States, the cost-recovery equilibrium sources training from the lowest-cost '
        'available producer and inference from '
        f'{_iso_name.get(usa_inf, usa_inf)}. '
    )
    if gbr_inf == deu_inf and gbr_inf != 'GBR':
        # Germany, France, and the UK share a single inference hub
        p.add_run(
            'For Germany, France, and the United Kingdom, inference is sourced from '
    ...
    p.add_run(
        'For China, the lowest-cost foreign inference supplier is '
        f'{_iso_name.get(chn_inf, chn_inf)}. '
        'Inference supply therefore clusters around latency-constrained regional '
        'hubs, with each major market served by a nearby low-cost producer.'
    )

    # v31: New paragraph — China DD regime illustration
    p, cur = mkp(doc, body, cur)
    p.add_run(
        'Among these demand centers, China illustrates the DD regime. China '
        'hosts one of the world\u2019s largest data-center markets. Yet almost all '
        'capacity is built and financed by domestic '
        'technology firms and state-owned operators, including Alibaba Cloud, Tencent, '
        'Baidu, and China Telecom (Aykut et al. 2026). '
        ...
        'Data, Western Computing project further support domestic sourcing by shifting '
        'energy-intensive computing to western provinces with cheaper power.'
    )
    ...
    if es10 < 0.005:
        p.add_run(
            ...
            'even a 10 percent uniform sovereignty premium is sufficient to make domestic '
            ...
            f'Raising the uniform premium to 20 percent shifts {_num_word(extra)} additional '
            f'{"country" if extra == 1 else "countries"}'
            f'{_name_frag} to domestic '
            'production, but the marginal effect is small. '
            'Inference exports respond less to sovereignty premia because '
            'nearby suppliers retain a latency advantage.'
        )
    else:
        p.add_run(
            f'Raising the uniform premium to 20 percent shifts {_num_word(extra)} '
            ...
            f'from {es10 * 100:.0f} percent to '
            f'{es20 * 100:.0f} percent. '
            'Inference exports respond less to sovereignty premia because '
            'nearby suppliers retain a latency advantage.'
        )
    ...
    p.add_run(
        ...
        f'The demand-weighted welfare cost is {demand_data["welfare_pct"]:.1f} percent of '
        'total compute spending, in the range of the welfare gains from trade '
        'in goods estimated in the quantitative trade literature, which range '
        'from roughly 0.2 percent to 10 percent depending on country size and specification '
        '(Eaton and Kortum 2002, Arkolakis et al. 2012).'
        ' At current demand levels (approximately 6\u2009\u00d7\u200910\u00b9\u2070 GPU-hours at '
        '$1.50/hr), this amounts to roughly $1.44\u2009billion per year. At this scale, '
        'governments can rationally choose domestic production for sensitive '
        'workloads. Examples include military '
        'applications, health records, and national statistical systems.'
    )
    ...
    p.add_run(
        ...
        'even commercial buyers have reason to '
        'choose domestic processing over cheaper foreign supply.'
    )

    p, cur = mkp(doc, body, cur)
    p.add_run(
        'Sovereignty preferences are not limited '
        'to developing countries: France\u2019s SecNumCloud certification '
        'mandates EU-only data hosting and staffing, effectively excluding '
        'U.S. hyperscalers from sovereign workloads. Developing countries '
        'that adopt broad data localization requirements risk foreclosing '
        'both import savings and regional export opportunities, the '
        'specialization gains that the model predicts. The World Bank (2025) '
        'frames this as a core policy trade-off: building domestic capacity '
        'versus securing affordable access to international cloud services.'
    )
    ...
    if sens:
        ...
        p.add_run(
            'Cost rankings are robust to substantial variations in parameters. '
        )
        ...
        p.add_run(
            ...
            'from domestic production to importing. This expands trade volumes '
            'but reduces exporter rents.'
        )
    ...
    omath(p, [_v('\u03C1')])
    p.add_run(
        ' by \u00b14 percent changes the composition of the top fifteen '
        'countries, but the qualitative result is stable across all '
    ...
    p.add_run(
        ...
        'preferred because it captures observed cross-country heterogeneity.'
    )
    ...
    p._element.append(make_hyperlink('Table3', 'Table 3'))
    p.add_run(
        ' omits a separate tiered column. Tiering primarily affects '
    ...
    p.add_run(
        ...
        'markups can raise effective GPU prices by 5\u201315 percent in developing '
        'countries that are not subject to export bans, eroding the thin '
        'cost advantages. These omitted constraints all work against '
        'developing-country competitiveness. The calibration results should '
    ...
    p.add_run(
        'The model treats electricity prices as fixed. In small countries '
        'with low-cost power, a hyperscale facility can be large relative '
        'to the host grid \u2014 a 100\u2009MW data center would consume '
        'roughly 3 percent of Kyrgyzstan\u2019s 3,800\u2009MW national output. At '
        'the multi-facility scale, data centers would compete with '
        'residential and industrial electricity demand, making regulatory '
        'intervention likely. The capacity ceiling '
    )
    omath(p, [_mbar_sub('K', 'j')])
    p.add_run(
    ...
    p._element.append(make_hyperlink('TableA1', 'Table A1'))
    p.add_run(
        ' are upper bounds. Allowing electricity supply curves to slope '
        'upward would narrow these apparent advantages. Evidence from the US '
        'suggests the AI '
        'workloads, combined with broader electrification, could raise '
        ...
        'local electricity prices by roughly 0.9 percent. Counties that have '
        'ever hosted a '
    ...
    # GPU upgrades (italic lead-in removed per user edit; Option B replaces the
    # "older hardware must discount" sentence with a financing/vintage mechanism)
    p, cur = mkp(doc, body, cur, space_before=6)
    p.add_run(
        'Newer GPUs, such as NVIDIA\u2019s B200, increase training throughput '
        'substantially but also raise power draw to roughly 1\u2009kW. Higher '
        'power draw makes electricity prices more important, which modestly '
        'strengthens the advantage of low-cost power producers. Because higher '
        'capital costs lead developing economies to refresh GPUs less '
        'frequently, they are more likely to operate older, less efficient '
        'hardware, raising their cost per effective FLOP and partly offsetting '
        'their electricity-cost advantage. The qualitative ranking is not driven '
    ...
    p.add_run(
        ...
        'This specification is standard in the services trade, but it simplifies how '
        'sovereignty frictions operate. Many sovereignty frictions are '
        'closer to fixed entry costs than to variable markups. Cloud security '
        'certifications, sanctions screening, export control compliance, '
        'and bilateral digital services agreements all raise the cost of '
        'entering a market. Our model also treats '
    )
    omath(p, [_msub('λ', 'jk')])
    p.add_run(
    ...
    omath(p, [_msub('λ', 'jk')])
    p.add_run(
        ' and fixed costs. Both extensions would likely make entry harder for '
        'developing-country exporters, so they should strengthen rather than '
        'weaken the main result.'
    )
    ...
    p.add_run(
        'Because hardware dominates the cost stack, financing terms can '
        'determine the ranking. At an 8 percent WACC, an OECD hyperscaler '
        'pays about $1.58 per GPU-hour in hardware costs. At an 18 percent '
        'WACC, a locally financed developing-country operator pays $1.87, '
        'a $0.29 gap on hardware alone (Calcaterra et al. 2024). '
        'Column (4) of '
    )
    p._element.append(make_hyperlink('Table3', 'Table 3'))
    p.add_run(
        ' reports the cost-recovery ranking with hardware amortized at '
        'each country\u2019s income-group WACC, using 8 percent for HICs, '
        '12 percent for UMICs, 15 percent for LMICs, and 18 percent for '
        'LICs. Once financing costs enter, cheap electricity explains less '
        'of the ranking, and several regime classifications shift.'
    )
    ...
    p.add_run(
        ...
        'partially bypasses the financial channel. If a multinational owns, '
        ...
        'country’s financial exposure. A fully hyperscaler-financed project '
        'is therefore best treated as a limiting case rather than the '
        'baseline. For this reason, Column (4) is the more realistic benchmark for '
        'assessing whether developing countries can capture compute-export '
        'rents with domestic capital.'
    )
    ...
    p.add_run(
        'Edge computing could relax one of the model\u2019s central '
        'constraints. As more AI demand comes from real-time inference on '
        'lightweight models '
        'deployed on local devices, the capacity constraint for '
        'inference becomes less binding: inference need not be concentrated in large '
        ...
        'inference exports may become less concentrated, and more '
        'countries could serve regional demand. For '
        ...
        '(Aykut et al. 2026). A fuller trade model could distinguish '
        'centralized GPU clusters for training from '
        'distributed edge nodes for inference.'
    )
...
def write_conclusion(doc, body, hmap, demand_data):
    ...
    # v34 (Edit C1): lead with the finding; the model description survives in
    ...
    p.add_run(
        ...
        'thin cost advantage, while the institutional frictions facing '
        'developing-country suppliers are often wider than that advantage.'
    )

    p, cur_concl = mkp(doc, body, cur_concl)
    p.add_run(
        'The 85-country calibration shows that hardware dominance compresses '
        'cross-country differences in production costs. Cheap electricity and '
        'favorable cooling can make countries cost-competitive quickly, but '
        'modest frictions, including a sovereignty premium, weak governance, or '
        'a higher cost of capital, can erase that advantage. Compute may '
        'therefore be unusually accessible on engineering grounds, yet unusually '
        'exposed to policy and financing frictions because the production-cost '
        'spread is narrow.'
    )
    ...
    p.add_run(
        ...
        'credibility eliminates much of the cost '
        'advantage that developing countries hold under pure cost-recovery '
        'pricing. The binding constraint is often not the electricity price '
        'itself but rather non-sanctioned status, credible long-term '
        'power contracts, stable network connectivity, enforceable data '
        'governance rules, and a regulatory environment '
        'that supports capital-intensive investment. '
        'Where those conditions are absent, buyers shift '
        'toward domestic production despite higher production costs. '
        'The resulting welfare loss is large in dollars but modest relative '
        f'to total compute spending, at about {demand_data["welfare_pct"]:.1f} percent, so '
        'governments with legitimate '
        'data-sovereignty objectives may rationally choose domestic supply. '
    )

    # v34 (Edit B): the policy-lever paragraph — financing, regulatory
    # credibility, rent capture (mechanisms from §7.1, §6.2/§7.2, Appendix D)
    p, cur_concl = mkp(doc, body, cur_concl)
    p.add_run(
        'For developing countries pursuing compute exports, the binding policy '
        'levers lie mostly outside the electricity tariffs. The first is '
        'financing. Because hardware dominates the cost stack, differences in '
        'the weighted average cost of capital can exceed the energy-cost '
        'advantage by several times. Sovereign guarantees, blended '
        'finance, multilateral risk insurance, and hyperscaler co-investment '
        'can therefore narrow the competitiveness '
        'gap more than any feasible reduction in '
        'electricity tariffs. The second lever is regulatory credibility. '
        'Data-adequacy status, mutual recognition agreements, and recognized '
        'cloud security certifications reduce the trust premium applied by '
        'foreign buyers. Broad data-localization mandates '
        'raise that premium and can foreclose export opportunities.'
    )

    p, cur_concl = mkp(doc, body, cur_concl)
    p.add_run(
        'The final policy question is how much surplus the host country '
        'captures. The Kyrgyzstan facility '
        'in Appendix D is NPV-positive, but under full foreign-hyperscaler '
        'ownership, the host country retains little beyond electricity payments '
        'and construction-phase jobs. The domestic share of operating surplus '
        'depends on taxation, equity participation, and royalty terms. '
        'Cheap energy can make a country cost-competitive, but '
        'rent capture depends on financing, governance, and ownership.'
    )

    p, cur_concl = mkp(doc, body, cur_concl)
    p.add_run(
        'The policy implications differ for training and inference. Restricting '
        'training imports raises costs without improving service quality, '
        'because training has no meaningful proximity advantage. Domestic '
        'inference production is easier to justify, since low latency can '
        'provide a genuine quality-of-service benefit, although that rationale '
        'weakens for countries located near low-cost regional hubs.'
    )

    # v34 user edit: new closing paragraph — capital-intensive resource
    # processing framing + resource-curse caveat + gravity-estimation outlook
    p, _ = mkp(doc, body, cur_concl)
    p.add_run(
        'FLOP exporting is best understood as capital-intensive resource '
        'processing: imported equipment converts local energy into an '
        'exportable digital service with minimal domestic labor input. Unlike '
        'extractive commodities, the energy input can be renewable, and '
        'demand for compute is growing faster than demand for most physical '
        'commodities. Still, the resource-curse literature remains relevant because '
        'concentrated compute-export revenues can generate exchange-rate pressure, '
        'institutional risks, and exposure to demand cycles. Whether '
        'these risks materialize depends on ownership structures, fiscal '
        'arrangements, and domestic rent capture. As bilateral compute-trade '
        'data become available, this framework can be estimated using '
        'gravity-style methods.'
    )
...
def write_appendix(doc, body, last_ref_el, eca_cal, non_eca_cal, reg, demand_data):
    ...
    a2_cw = [1500, 650, 550, 500, 650, 650, 550, 700, 750]
...
def write_table_a2(doc, body, after_el, demand_data):
    """Table A2: Complete country rankings under alternative cost and
    sovereignty specifications (landscape, 8 cols)."""
    ...
    # Sort all countries by spec (2) rank (cost-recovery)
...
def write_sensitivity_appendix(doc, body, last_el, demand_data):
    ...
    p._element.append(make_hyperlink('TableA3', 'Table A3'))
    p.add_run(
        ' reports cost-recovery rankings under three robustness specifications. '
        'The baseline uses the calibrated hardware cost; the alternative scenarios '
        'vary the hardware cost share to test sensitivity of the rankings.'
    )

    headers = ['Scenario', 'Parameter change', 'Dev top 15',
               'Max spread', 'Spearman \u03c1 vs baseline', 'Top 5']
    rows = []
    for s in sens:
        rows.append([
            s["label"],
    ...
    tbl_el = add_table(
        ...
        title='Table A3. Sensitivity of cost-recovery rankings to parameter variation',
    ...
    note.paragraph_format.line_spacing = 1.0
    rn = note.add_run(
        'Notes: Each row uses cost-recovery pricing (subsidized tariffs replaced with LRMC). '
        'Dev top 15 = number of developing countries in top 15. '
        'Max spread = maximum percentage difference between costliest and cheapest producer. '
        'Spearman \u03C1 vs baseline = rank correlation with baseline scenario.'
    )
    rn.font.size = Pt(10)
    note.alignment = WD_ALIGN_PARAGRAPH.LEFT
    note_el = note._element
...
def write_kyrgyzstan_appendix(doc, body, last_el):
    ...
    r.bold = True
    p.add_run(
        'Kyrgyzstan depends on the Toktogul reservoir for over 80\u2009percent '
        'of electricity (ADB 2020); seasonal drawdowns and drought years create '
...
def write_figure1_calibration(doc, body, last_ref):
    ...
    fig_path = DOCS.parent / "Output" / "figure1_model_structure.png"
    ...
    run_ft = title_p.add_run('. Model structure')
    run_ft.bold = True
...
def write_table1(doc, body, after_el):
    ...
    rn = tn.add_run(
        ...
        'in Proposition 1. Letter codes (EE, IE, ID, DD, II) denote training '
        'status (first letter: E\u2009=\u2009export, I\u2009=\u2009import, '
        'D\u2009=\u2009domestic) and inference status (second letter), used in '
    )
    rn.font.size = Pt(10)
...
def write_table2(doc, body, after_el, demand_data):
    ...
    _sym_map = {
        ...
        'phi': '\u03C6', 'delta': '\u03B4',
        'theta_bar': '\u03B8\u0304',  # rendered via OMML in cell
...
def write_table3(doc, body, after_el, demand_data):
    """Table 3: Country rankings under alternative cost and sovereignty
    specifications (landscape, 8 cols)."""
    ...
    rn3 = note.add_run(
        ...
        'that columns (1)\u2013(3) suppress by amortizing hardware at a single '
        'straight-line \u03c1 with no financing cost. '
...
def link_equations(body):
    ...
    count = 0
    bm_id_eq = [900]
    eq_pattern = re.compile(r'(?:equation|eq\.)[\s\u2009]\((\d+)\)')
    for p_el in list(body.findall(qn('w:p'))):
        for child in list(p_el):
            if child.tag != qn('w:r'):
                continue
    ...
    for p in doc.paragraphs:
        ...
        if style == 'Heading 1':
            for run in p.runs:
                ...
                run.font.size = Pt(14)
                # Bold comes from the Heading 1 style (set above); do NOT set it
                # directly on the run. Word strips a run-level <w:b> that is
                # redundant with the style on save, which otherwise shows up as a
                # spurious format diff on every heading during edit integration.
                run.bold = None
            continue
        ...
        if 'Heading' not in style and p.text.strip():
            ...
            if (
                p._element not in _protected
                and p.alignment != WD_ALIGN_PARAGRAPH.LEFT
            ):
                p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            ...
            if p._element in _protected:
                continue
            # Preserve table note / cell formatting (explicit line_spacing)
            ls = p.paragraph_format.line_spacing
            if ls is not None and ls != 1.5 and ls <= 1.0:
                continue
            # 1.5 line spacing for body paragraphs
            p.paragraph_format.line_spacing = 1.5
            # Post-equation paragraph: previous sibling is a w:tbl
            prev_sib = p._element.getprevious()
            is_post_eq = (
                prev_sib is not None and prev_sib.tag == qn('w:tbl')
            )
            if is_post_eq:
...
def main():
    ...
    (p_T_cr, _, shares_cr, _, _, _, _
     ) = solve_capacity_equilibrium(0.0, "\u03bb=0 CR")

    # ═══════════════════════════════════════════════════════════════════════
    # EQUILIBRIUM ON COST-RECOVERY BASELINE (no ξ adjustment)
    # ═══════════════════════════════════════════════════════════════════════
    print("Computing equilibrium on cost-recovery baseline...")

    # Use cost-recovery supply stack permanently (no efficiency adjustment)
    supply_stack = cr_supply_stack
    costs_dict = costs_dict_cr

    # Run capacity equilibrium on cost-recovery costs
    ...
    for iso_k in dc_k:
        ...
        for tier in (1, 2, 3):
            ...
            for iso_j, c_j in adj_costs.items():
                ...
                if l_jk is None:
                    continue
                cost_del = (1 + lam_eff) * (1 + TAU * l_jk) * c_j
    ...
    for d in table3_data:
        ...
        for tier, w_t in [(2, W_TIER2), (3, W_TIER3)]:
            ...
            lam_star_k = d["cj_cr"] / p_T_tiered - 1 if p_T_tiered > 0 else 0
            if lam_k >= lam_star_k or d["cj_cr"] <= p_T_tiered:
                dom_share += w_t  # domestic for this tier
        d["dom_share_tiered"] = dom_share

    # Spec (5): Uniform λ = 10% (comparison with old specification)
    p_star = sorted(table3_data, key=lambda x: x["cj_cr"])[4]["cj_cr"]
    for d in table3_data:
        d["sigma_bar"] = max(0, (d["cj_cr"] - p_star) / p_star)
        if d["sigma_bar"] <= 0.10:
            d["pj_sov"] = d["cj_cr"]   # domestic
            d["type_uniform"] = "DD" if d["rank_cr"] > 5 else "EE"
        else:
    ...
    # Rank spec (5) — tiebreak importers by cj_cr
    ...
    # Store top-5 CR ranking for prose references
    t3_sorted_cr = sorted(table3_data, key=lambda x: x["cj_cr"])[:5]
    demand_data["cr_top5"] = [(d["country"], d["cj_cr"]) for d in t3_sorted_cr]

    print(f"  Table 3: {len(table3_data)} countries, p* = ${p_star:.4f}/hr")
    _raw_top5 = [
        d['country']
        for d in sorted(table3_data, key=lambda x: x['cj_raw'])[:5]
    ]
    _cr_top5 = [
        d['country']
        for d in sorted(table3_data, key=lambda x: x['cj_cr'])[:5]
    ]
    print(f"  Spec (1) top 5: {_raw_top5}")
    print(f"  Spec (2) top 5: {_cr_top5}")
    ...
    n_dom_uniform = sum(
        1 for d in table3_data if d.get("type_uniform") == "DD"
    )
    print(
        f"  Domestic: bilateral={n_dom_bilat}, tiered={n_dom_tiered}, "
        f"uniform={n_dom_uniform}"
    )
    ...
    print(f"  EE counts: raw={n_raw_ee}, cr={n_cr_ee}, bilat={n_bilat_ee}")
    ...
    # Skipped in --anon mode: the anonymized file is a derived submission
    # artifact, not a canonical paper version, and must not enter git history.
    # ═══════════════════════════════════════════════════════════════════════
    if NO_COMMIT:
        print("Skipping git auto-commit (--anon/--no-commit)." )
        return
    try:
        ...
        result = subprocess.run(
            ['git', 'commit', '-m', f'Auto-save v34: {_ts}'],
