"""
Produce flop_trade_model_v33.docx from v8.docx.
...
def write_title_and_abstract(doc, body, all_el, hmap, demand_data=None):
    ...
    r_ver = ver_p.add_run(f'v33  \u2014  {datetime.now().strftime("%B %d, %Y  %H:%M")}')
...
def write_introduction(doc, body, hmap):
    ...
    p.add_run(
        ...
        'capital between OECD hyperscalers (8 percent) and locally financed developing-country '
        'operators (18 percent) adds roughly $0.29/GPU-hour to unit cost \u2014 about four times '
...
def write_calibration(doc, body, hmap, cal, reg, n_eca, n_total, demand_data):
    ...
    p.add_run(
        ...
        '$1.50/hr), this amounts to roughly $1.4\u2009billion per year. This cost is small '
...
def write_conclusion(doc, body, hmap, demand_data):
    ...
    p.add_run(
        ...
        'toward domestic production. That loss is sizable in absolute dollars '
        'but modest relative to compute spending, and small enough that '
...
def main():
    ...
    out = DOCS / "flop_trade_model_v33.docx"
    ...
    try:
        ...
        result = subprocess.run(
            ['git', 'commit', '-m', f'Auto-save v33: {_ts}'],
