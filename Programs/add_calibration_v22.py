...
def write_calibration(doc, body, hmap, cal, reg, n_eca, n_total, demand_data):
    ...
    p.add_run(
        ...
        f'With a {LAMBDA:.0%} sovereignty premium, most large economies shift to full '
        'domestic production.'
    )
...
def write_table_a2(doc, body, after_el, demand_data):
    ...
    run_t = tp.add_run('. Country rankings under alternative pricing assumptions (all countries)')
    run_t.bold = True
    ...
    _mc(0, 7, 9)
    _sc(0, 7, '(3) Reliability-Adj.', bold=True, fs=7)
    _mc(0, 10, 12)
    _sc(0, 10, '(4) Sovereignty (\u03c3\u0304=10%)', bold=True, fs=7)
    _sc(0, 13, '', fs=7)
    ...
    for i, d in enumerate(all_sorted):
        ...
        _sc(ri, 10, f'${d["pj_sov"]:.2f}', fs=7)
        _sc(ri, 11, str(d["rank_sov"]), fs=7)
        _sc(ri, 12, d["type_sov"], fs=7)
        dv = d["delta"]
...
def write_table3(doc, body, after_el, demand_data):
    """Table 3: Country Rankings Under Alternative Pricing Assumptions (landscape)."""
    ...
    # Merge cols 7-9: "(3) Reliability-Adj."
    _merge_cells(0, 7, 9)
    _set_cell(0, 7, '(3) Reliability-Adj.', bold=True, font_size=8)
    # Merge cols 10-12: "(4) Sovereignty (\u03c3\u0304=10%)"
    _merge_cells(0, 10, 12)
    _set_cell(0, 10, '(4) Sovereignty (\u03c3\u0304=10%)', bold=True, font_size=8)
    ...
    rn3 = note.add_run(
        ...
        '(3)\u2009Reliability-adjusted: cost-recovery price divided by institutional reliability '
        'index \u03be\u2c7c (preferred specification). '
...
def main():
    ...
    for iso_k in dc_k:
        ...
        if c_k is None:
            continue
        is_dom_train = (c_k <= (1 + LAMBDA) * p_T_0)
        is_dom_inf = (adj_reg.get(iso_k, {}).get('best_inf_source') == iso_k)
        r = classify_regime_5type(iso_k, train_exporter_isos,
                                  inf_exporter_isos, is_dom_train, is_dom_inf)
        regime_5[iso_k] = r
        _reg_5type[r] += 1

    demand_data["regime_5"] = regime_5
    demand_data["reg_5type_counts"] = _reg_5type
    print(f"  5-type regimes: {dict((k, v) for k, v in _reg_5type.items() if v)}")
    ...
    # Compute full unit costs for all 4 specifications
    for d in table3_data:
        d["cj_raw"] = d["elec_raw"] + rho_hw + d["constr_cost"] + rho_net    # (1)
        d["cj_cr"] = d["elec_cr"] + rho_hw + d["constr_cost"] + rho_net      # (2)
        d["cj_eff"] = d["cj_cr"] / d["xi"] if d["xi"] > 0 else 999          # (3)
    ...
    # Spec (4): Sovereignty — p* = 5th cheapest under spec (3)
    ...
    # Rank spec (4) — tiebreak importers by cj_eff
    ...
    for d in table3_data:
        ...
        if d["rank_eff"] <= 5:
            d["type_sov"] = "EE"
        elif d["sigma_bar"] <= 0.10:
            d["type_sov"] = "DD"
        else:
            d["type_sov"] = "II"
        # Delta: rank improvement from raw to reliability-adjusted
