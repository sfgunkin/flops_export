"""
Produce flop_trade_model_v24.docx from v8.docx.
...
def compute_xi_eff(governance_score, grid_score):
    """Compute production-efficiency index ξ_j^{eff}.

    Grid reliability × operational risk. Sanctions excluded.
    governance_score: WGI Rule of Law percentile [0, 1] (operational risk)
    grid_score: inverse of hours without electricity [0, 1] (grid reliability)
    """
    return governance_score * grid_score
...
def write_title_and_abstract(doc, body, all_el, hmap):
    ...
    r_ver = ver_p.add_run(f'v24  \u2014  {datetime.now().strftime("%B %d, %Y  %H:%M")}')
...
def write_trade_costs(doc, body, hmap):
    ...
    omath(p, [_v('j')])
    p.add_run(
        '. It is constructed as the product of grid reliability '
        '(inverse of hours without electricity per firm per year) and operational risk '
        '(a composite of contract enforcement, expropriation risk, and regulatory '
        'predictability). It approaches one for countries with reliable power and strong '
        'institutions, and falls below one for countries where outages or operational risk reduce '
        'effective delivery. Sanctions, data-protection trust, and regulatory compatibility '
        'do not enter '
    )
...
def write_calibration(doc, body, hmap, cal, reg, n_eca, n_total, demand_data):
    ...
    omath(p, [_msubsup('\u03BE', 'j', 'eff')])
    p.add_run(
        ' (equation (3)) is constructed as the geometric mean of the '
        'governance quality (World Bank Worldwide Governance Indicators, Rule of Law '
        'percentile, rescaled to [0, 1]) and grid reliability (inverse of hours without electricity '
        'per firm per year, from the World Bank Enterprise Surveys). '
        'For OECD countries with negligible outages, '
    )
    omath(p, [_msubsup('\u03BE', 'j', 'eff'), _t(' \u2248 1')])
    p.add_run(
        '. Developing countries with weak grids and governance face steeper penalties '
        '(e.g., Kyrgyzstan: 0.57, Iran: 0.51). '
        'Note that even countries with '
    )
    omath(p, [_msubsup('\u03BE', 'j', 'eff'), _t(' \u2248 1')])
    p.add_run(
        ' can shift in rank because penalizing low-\u03BE '
        'competitors mechanically raises other countries. '
        '\u03BE ranges from 0.95 to 1.00 even within the OECD, and the tight '
    ...
    # ── B2. Endogenous electricity prices (condensed) ──
    p, cur = mkp(doc, body, cur, space_before=6)
    ...
    p.add_run(
        ...
        'baseline rankings hold. If locally financed, '
        'the same institutional weaknesses that lower \u03BE '
    ...
    # ── B5. Sensitivity analysis (moved, slightly reworded) ──
...
def write_table2(doc, body, after_el, demand_data):
    ...
    rn1 = note.add_run(
        ...
        'The production-efficiency index \u03BE\u2C7C\u1d49\u1da0\u1da0 combines governance quality '
        'and grid reliability (equation 3). Sanctions exposure is captured by the bilateral '
...
def main():
    ...
    # v24: Production-efficiency index ξ_j^{eff} ∈ (0, 1]
    # Constructed from grid reliability × operational risk (no sanctions)
    xi = {}          # new ξ_j^{eff}
    ...
    with open(DATA / "reliability_index.csv", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            iso = row["iso3"]
            gov = float(row["governance"])     # operational risk [0,1]
            grid = float(row["grid_quality"])  # grid reliability [0,1]
            xi_old[iso] = float(row["xi_reliability"])
            # ξ_j^{eff} = geometric mean of grid reliability and operational risk
            xi[iso] = (gov * grid) ** 0.5 if (gov * grid) > 0 else 0.01
    ...
    out = DOCS / "flop_trade_model_v24.docx"
