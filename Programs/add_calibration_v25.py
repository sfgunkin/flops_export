...
def compute_xi_eff(governance_score, grid_score):
    ...
    Weighted geometric mean: governance^ω × grid^(1−ω).
    ω = 0.85 reflects enclave character of data center operations.
    governance_score: WGI Rule of Law percentile [0, 1]
    grid_score: inverse of hours without electricity [0, 1]
    """
    return (governance_score ** OMEGA_XI) * (grid_score ** (1 - OMEGA_XI))
...
def write_title_and_abstract(doc, body, all_el, hmap):
    ...
    r_ver = ver_p.add_run(f'v25  \u2014  {datetime.now().strftime("%B %d, %Y  %H:%M")}')
...
def write_trade_costs(doc, body, hmap):
    ...
    omath(p, [_v('j')])
    p.add_run(
        '. It is a weighted geometric mean of governance quality and grid reliability: '
    )
...
def write_calibration(doc, body, hmap, cal, reg, n_eca, n_total, demand_data):
    ...
    # Production-efficiency index (enclave argument, ω = 0.85)
    p, cur = mkp(doc, body, cur)
    add_italic(p, 'Production efficiency. ')
    p.add_run('The production-efficiency index ')
    omath(p, [_msubsup('\u03BE', 'j', 'eff')])
    p.add_run(
        ' from equation (3) is a weighted geometric mean of governance quality and grid '
        'reliability with weight '
    )
    omath(p, [_v('\u03C9'), _t(' = 0.85')])
    p.add_run(
        ' on governance. The asymmetric weighting reflects the enclave character of '
    ...
    # ── B5b. Efficiency weight ω robustness ──
    p, cur = mkp(doc, body, cur, space_before=6)
    add_italic(p, 'Efficiency weight. ')
    p.add_run('The baseline assigns weight ')
    omath(p, [_v('\u03C9'), _t(' = 0.85')])
    p.add_run(
        ' to governance and 0.15 to grid reliability. '
        'Table [X] reports results for '
    )
    omath(p, [_v('\u03C9'), _t(' = 0.70, 0.85,')])
    p.add_run(' and ')
    omath(p, [_t('1.00')])
    p.add_run(
        '. At '
    )
    omath(p, [_v('\u03C9'), _t(' = 1.00')])
    p.add_run(
        ' (governance only), grid reliability drops out entirely; at '
    )
    omath(p, [_v('\u03C9'), _t(' = 0.70')])
    p.add_run(
...
def main():
    ...
    # v25: Production-efficiency index ξ_j^{eff} ∈ (0, 1]
    # Weighted geometric mean: governance^ω × grid^(1−ω), ω = OMEGA_XI = 0.85
    xi = {}          # new ξ_j^{eff}
    ...
    with open(DATA / "reliability_index.csv", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            # ξ_j^{eff} = gov^ω × grid^(1−ω)
            xi[iso] = compute_xi_eff(gov, grid) if (gov > 0 and grid > 0) else 0.01
    ...
    out = DOCS / "flop_trade_model_v25.docx"
