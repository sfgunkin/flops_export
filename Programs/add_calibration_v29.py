"""
Produce flop_trade_model_v29_base.docx from v8.docx.
...
v29: Fresh pipeline numbers — explicit η, no residual, no ξ floor:
  ...
  - Dynamic top-25 selection (sorted by rank_eff, no hardcoded list)
  - Inline sensitivity computation (5 scenarios, no external xlsx)
...
def run_sensitivity(cal, omega, dc_k, k_bar, sanctioned, xi_vals, xi_raw_vals):
    """v29: Inline sensitivity computation (5 scenarios, no external xlsx).

    Recomputes cost rankings under alternative parameter assumptions.
    No floor scenarios (v29 has no floor); Form A uses ω=0.85.

    Returns list of scenario result dicts for Table A3.
    """
    import openpyxl

    # Load xi components (governance + grid) for re-weighting
    ...
    for _xi_row in _xi_ws.iter_rows(min_row=2, values_only=True):
        ...
        iso = _xi_d["ISO3"]
        gov = float(_xi_d["G_RoL"])
        grid = float(_xi_d["R_grid"])
        xi_comp[iso] = (gov, grid)
    _xi_wb.close()

    # 5 scenarios for Table A3
    scenario_defs = [
        ('Baseline (\u03C9=0.50, \u03C1=$1.36, Form B)',
         '\u03C9=0.50, \u03C1=$1.36', 0.50, RHO, 'B'),
        ('High governance weight (\u03C9=0.85)',
         '\u03C9=0.85', 0.85, RHO, 'B'),
        ('Low hardware share (\u03C1=$1.30)',
         '\u03C1=$1.30 (\u22124.4%)', 0.50, 1.30, 'B'),
        ('High hardware share (\u03C1=$1.42)',
         '\u03C1=$1.42 (+4.4%)', 0.50, 1.42, 'B'),
        ('Form A (v26 specification, \u03C9=0.85)',
         'c_adj = c_cr / \u03BE_eff, \u03C9=0.85', 0.85, RHO, 'A'),
    ]

    results = []
    baseline_top5 = None
    cr_ranks = {}  # cost-recovery ranks for Spearman

    for label, param_change, omega_val, rho_val, form in scenario_defs:
        ranked = []
        for r_row in cal:
            iso = r_row["iso3"]
            ...
            gov, grid = xi_comp.get(iso, (0.5, 0.5))
            xi_j = (gov ** omega_val) * (grid ** (1 - omega_val)) if gov > 0 and grid > 0 else 0.01

            if form == 'B':
                c_adj = rho_val + (c_cr - rho_val) / xi_j if xi_j > 0 else 999
            else:  # Form A
                c_adj = c_cr / xi_j if xi_j > 0 else 999

            ranked.append({"iso": iso, "country": r_row["country"],
                           "c_cr": c_cr, "c_adj": c_adj, "xi": xi_j})
        ...
        # CR ranks (same for all scenarios with same rho)
        ...
        # Max markup: max (c_adj - c_cr) / c_cr
        max_markup = max((d["c_adj"] - d["c_cr"]) / d["c_cr"] * 100
                         for d in ranked if d["c_cr"] > 0)
        # Spearman rank correlation vs cost-recovery
        n = len(ranked)
        d_sq = sum((d["rank_adj"] - cr_ranks.get(d["iso"], d["rank_adj"])) ** 2
                    for d in ranked)
...
def write_introduction(doc, body, hmap):
    ...
    p.add_run(
        ...
        'Some of the cheapest producers drop sharply, while countries combining cheap energy '
        'with adequate institutions remain in the top 15. '
        'Under the bilateral sovereignty specification, however, trust deficits eliminate '
        ...
        'and five developing economies re-enter as potential exporters. '
...
def write_trade_costs(doc, body, hmap):
    ...
    # Split: efficiency adjustment in new paragraph
    p, cur = mkp(doc, body, cur)
    p.add_run(
...
def write_calibration(doc, body, hmap, cal, reg, n_eca, n_total, demand_data):
    ...
    # v29: count developing countries in top 15
    _t3_eff_sorted = sorted(_t3, key=lambda x: x["rank_eff"])
    _dev_top15 = [d for d in _t3_eff_sorted[:15] if d["iso"] in DEVELOPING]
    _n_dev_top15 = len(_dev_top15)
    p.add_run(
        'All top-fifteen producers are OECD member states; '
        f'China, the highest-ranked developing country, appears at rank\u2009{next(d["rank_eff"] for d in _t3_eff_sorted if d["iso"] == "CHN")}. '
        ...
        f'while Denmark and Iceland rise {abs(_dnk_delta)} places. '
        'Countries that combine cheap energy with strong institutions retain their '
    ...
    p, cur = mkp(doc, body, cur)
    # v29: dynamically find small-capacity countries in top 20 and large-capacity outliers
    _dc_k = demand_data["dc_k"]
    _eff_top20 = _t3_eff_sorted[:20]
    _small_cap = [(d["country"], d["rank_eff"], int(_dc_k.get(d["iso"], 5)))
                  for d in _eff_top20 if _dc_k.get(d["iso"], 5) < 100]
    _small_cap_str = ', '.join(
        f'{co} (rank\u2009{rk}, {cap}\u2009MW)' for co, rk, cap in _small_cap)
    # High-cost countries with large capacity
    _nld_rank = next(d["rank_eff"] for d in _t3_eff_sorted if d["iso"] == "NLD")
    _irl_rank = next(d["rank_eff"] for d in _t3_eff_sorted if d["iso"] == "IRL")
    _sgp_rank = next(d["rank_eff"] for d in _t3_eff_sorted if d["iso"] == "SGP")
    ...
    p.add_run(
        ...
        f'Among the twenty cheapest producers after efficiency adjustment, '
...
def write_sensitivity_appendix(doc, body, last_el, demand_data):
    ...
    p._element.append(make_hyperlink('TableA3', 'Table A3'))
    p.add_run(
        ' reports equilibrium outcomes under five robustness specifications. '
        ...
        'weight, hardware cost share, and functional form.'
    )
...
def write_table3(doc, body, after_el, demand_data):
    ...
    # ─── Row selection: top 25 by efficiency-adjusted rank (dynamic) ───
    top_eff_sorted = sorted(table3_data, key=lambda x: x["rank_eff"])
    _show = [d["iso"] for d in top_eff_sorted[:25]]
    _show_set = set(_show)
    top_rows = top_eff_sorted[:25]
...
def main():
    ...
    out = DOCS / "flop_trade_model_v29_base.docx"
    ...
    try:
        ...
        result = subprocess.run(
            ['git', 'commit', '-m', f'Auto-save v29: {_ts}'],
