"""
Produce flop_trade_model_v20.docx from v8.docx.
...
v20: Capacity-Constrained Ricardian Model restructuring.
  ...
  - 7 main-text display equations, 5 propositions
"""
...
CITATIONS = [
    ...
    ('Costinot, Donaldson, and Komunjer', '2012', 'CDK2012', 'Costinot, A.'),
...
ITALIC_IN_REFS = {
    ...
    'Costinot, A., J. Donaldson': 'Review of Economic Studies',
...
def write_title_and_abstract(doc, body, all_el, hmap):
    ...
    r_ver = ver_p.add_run(f'v20  \u2014  {datetime.now().strftime("%B %d, %Y  %H:%M")}')
...
def write_production_technology(doc, body, hmap):
    ...
    p, cur = mkp(doc, body, cur)
    p.add_run(
        'The production of compute at scale takes place in data centers that house thousands of '
        'GPU-equipped servers. Consider '
    )
    omath(p, [_v('N')])
    p.add_run(
...
def write_sourcing_and_equilibrium(doc, body, hmap, demand_data):
    ...
    # Equation (6): p_T = c_{(m_T)}  [was Eq 7]
    ...
    # nested m_T subscript inside the subscript
...
def write_equilibrium_properties(doc, body, hmap, demand_data):
    ...
    for t in s4.findall(f'.//{qn("w:t")}'):
        if t.text and 'Comp' in t.text:
            t.text = t.text.replace('Comparative Advantage', 'Equilibrium Properties')
            break
...
def write_data_section(doc, body, hmap, demand_data):
    ...
    p.add_run(
        ...
        'construction costs are predicted using a log-linear regression of ln($/W) on '
        'ln(GDP per capita), urban population share, a seismic zone indicator, and '
        'World Bank regional dummies ('
    )
    omath(p, [_msup('R', '2'), _t(' = 0.48')])
    p.add_run(
        '). The moderate explanatory power reflects the fact that construction costs depend on '
        'factors such as labor markets, building codes, and imported-materials '
        'logistics that these variables capture imperfectly. '
        'Since construction is only 3\u20136% of total FLOP costs, imputation '
        'error has limited impact on cost rankings.'
    )
    make_footnote(p,
...
def write_calibration(doc, body, hmap, cal, reg, n_eca, n_total, all_reg, all_sov, demand_data):
    ...
    p.add_run(f' countries ({n_eca} in ECA, {n_total - n_eca} non-ECA comparators), '
              'in the spirit of the multi-country Ricardian tests of '
    ...
    # Key findings — open with Table A2 reference
    p, cur = mkp(doc, body, cur, space_before=6)
    ...
    omath(p, [_v('N'), _t(f' = {n_total}')])
    p.add_run(' countries, sorted by unit cost under observed electricity prices. ')
    cheapest = cal[0]
    p.add_run(
    ...
    p.add_run(
        ...
        'even a small domestic preference tips the decision away from importing.'
    )
    ...
    p.add_run(
        ...
        f'additional {"country" if demand_data["extra_dom"] == 1 else "countries"} '
    ...
    p.add_run(
        ...
        'Grid reliability further narrows the set of viable exporters. '
    ...
    omath(p, [_msubsup('c', 'j', 'eff'), _t(' = '), _msub('c', 'j'),
              _t(' / '), _msub('\u03BE', 'j')])
    p.add_run('. ')
    # Compute reliability-adjusted rankings
    _xi_adj = demand_data.get("xi_adjusted", {})
    if _xi_adj:
        _xi_top5 = _xi_adj["top5"]
        _xi_n_changed = _xi_adj["n_changed_top10"]
        p.add_run(
            f'Under reliability adjustment, the five cheapest producers become '
    ...
    p.add_run(
        ...
        f'{_num_word(demand_data["regime_changes"])} '
...
def write_references(doc, body, refs):
    ...
    new_refs = [
        ...
        'Costinot, A., J. Donaldson, and I. Komunjer. (2012). \u201CWhat Goods Do Countries '
        'Trade? A Quantitative Exploration of Ricardo\u2019s Ideas.\u201D '
...
def main():
    ...
    out = DOCS / "flop_trade_model_v20.docx"
